...
from find_exit import find_the_exit

import cv2
import numpy as np
from Packages.djitellopy import Tello
...
class Drone(object):
    def __init__(self):
        ...
        if battery > 30:
            Thread(target=os.system, args=(
                "gnome-terminal -x ~/ORB_SLAM2/Examples/Monocular/tello ~/ORB_SLAM2/Vocabulary/ORBvoc.txt ~/ORB_SLAM2/Examples/Monocular/tello.yaml",),
                   daemon=True).start()
    ...
    def stay_in_the_air(self, is_exit_or_polygon=True):
        if is_exit_or_polygon:
            Thread(target=self.get_exit_points).start()
        else:
    ...
    def get_polygon_checkpoints(self):
        # get_polygon_checkpoints(self.get_newest_map_name(), True) # your own point
        self.polygon_checkpoints = [find_the_exit(self.get_newest_map_name())]
        self.exit_stay_in_the_air_loop = True
